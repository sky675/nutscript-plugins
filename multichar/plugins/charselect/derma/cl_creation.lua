...
function PANEL:Init()
	...
	self.model.OnRemove = function(modelp)
		if(modelp.modelcache and modelp.modelcache != 0) then
			for k,v in pairs(modelp.modelcache) do
				v:Remove()
				modelp.modelcache[k] = nil --srsly it doesnt get removed from the table
			end
		end
	end
