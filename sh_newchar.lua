...
--change bots to the tiny 1x1 boxes i guess? idk
 ...
 defaultfembot = "models/sky/legs/female_citizen1.mdl"
 defaultmaletop = "models/sky/seperate/male_anorak.mdl"
 defaultmalebot = "models/sky/legs/male_citizen1.mdl"

--this was originally just for gs models, so exec isnt really useful anymore,
--however fems is still useful
...
do
	...
	--overwriting the isFemale func
	--i originally just did this in ns itself but this SHOULD work too, 
	--untested, if it doesnt just find this function in ns and replace it
	function playerMeta:isFemale()
		local model = self:GetModel():lower()

		local custom = hook.Run("CustomFemaleModel", model, self) --it honestly should have something like this by default, not every female model meets the expecations below
...
hook.Add("CustomFemaleModel", "gs3customs", function(model, ply)
...
if(CLIENT) then
	--UNUSED CODE START
	--[[
	hook.Add("SetModel", "RemoveGSSupp", function(ply, mdl)
		if(!nut.newchar.isBM(model)) then
			if(g_Legs and g_Legs.LegEnt.bm and #g_Legs.LegEnt.bm != 0) then
				for k,v in pairs(g_Legs.LegEnt.bm) do
					v:Remove()
					g_Legs.LegEnt.bm[k] = nil
				end
			end
		end
	end)
	...
	function BMDoFinalRender()
		--print(tostring(g_Legs), tostring(g_Legs.LegEnt), tostring(g_Legs.LegEnt.bm))
		if(!g_Legs or !g_Legs.LegEnt or !g_Legs.LegEnt.bm) then return end
		cam.Start3D(EyePos(), EyeAngles())
			 if ShouldDrawLegs() then
				--print("but can you do this")
 
				 local bEnabled = render.EnableClipping(true)
					 render.PushCustomClipPlane(g_Legs.ClipVector, g_Legs.ClipVector:Dot(EyePos()))
						 render.SetColorModulation(g_Legs.RenderColor.r / 255, g_Legs.RenderColor.g / 255, g_Legs.RenderColor.b / 255)
							 render.SetBlend(g_Legs.RenderColor.a / 255)
							 		--print(g_Legs.RenderPos)
									 g_Legs.LegEnt.bm.t:SetRenderOrigin(g_Legs.RenderPos)
									 g_Legs.LegEnt.bm.t:SetRenderAngles(g_Legs.RenderAngle)
									 g_Legs.LegEnt.bm.t:SetupBones()
									 g_Legs.LegEnt.bm.t:DrawModel()
									 g_Legs.LegEnt.bm.t:SetRenderOrigin()
									 g_Legs.LegEnt.bm.t:SetRenderAngles()
									 g_Legs.LegEnt.bm.b:SetRenderOrigin(g_Legs.RenderPos)
									 g_Legs.LegEnt.bm.b:SetRenderAngles(g_Legs.RenderAngle)
									 g_Legs.LegEnt.bm.b:SetupBones()
									 g_Legs.LegEnt.bm.b:DrawModel()
									 g_Legs.LegEnt.bm.b:SetRenderOrigin()
									 g_Legs.LegEnt.bm.b:SetRenderAngles()
							 render.SetBlend(1)
	 ...
	 --[[
	 hook.Add("PostDrawTranslucentRenderables", "myturn", function()
		  if (LocalPlayer() && !LocalPlayer():InVehicle()) then
			 BMDoFinalRender()
		 end
	 end)
 
	 hook.Add("RenderScreenspaceEffects", "myturn", function()
	...
	--i hope this works lmao (it dont :()
	function nut.newchar.legsSupp(data)
		local ply = LocalPlayer()
		print("its being called")

		local setthistime = false

		print(tostring(g_Legs.LegEnt), 
		tostring(g_Legs.LegEnt.bm), 
		tostring(!g_Legs.LegEnt.bm), 
		tostring(g_Legs.LegEnt.bm and !IsValid(g_Legs.LegEnt.bm.t)), 
		tostring(IsValid(g_Legs.LegEnt) and (!g_Legs.LegEnt.bm or !IsValid(g_Legs.LegEnt.bm.t))))

		if(IsValid(g_Legs.LegEnt) and (!g_Legs.LegEnt.bm or !IsValid(g_Legs.LegEnt.bm.t))) then --if the models dont exist make them
			g_Legs.LegEnt.bm = {}
			setthistime = true
			print("its made")

			local t = ClientsideModel(ply.bm.t:GetModel())
			local b = ClientsideModel(ply.bm.b:GetModel())
			
			t:InvalidateBoneCache()
			t:SetParent(g_Legs.LegEnt)
			g_Legs.LegEnt:CallOnRemove("remtop", function(ent) t:Remove() end)
			t:AddEffects(bit.bor(EF_BONEMERGE,EF_BONEMERGE_FASTCULL,EF_PARENT_ANIMATES))
			t:SetupBones()
			t:SetNoDraw(true)
			g_Legs.LegEnt.bm.t = t

			b:InvalidateBoneCache()
			b:SetParent(g_Legs.LegEnt)
			g_Legs.LegEnt:CallOnRemove("rembot", function(ent) b:Remove() end)
			b:AddEffects(bit.bor(EF_BONEMERGE,EF_BONEMERGE_FASTCULL,EF_PARENT_ANIMATES))
			b:SetupBones()
			b:SetNoDraw(true)
			g_Legs.LegEnt.bm.b = b
			--[[
			function t:Think()
		...
		if(IsValid(g_Legs.LegEnt)&&data.reset&&!setthistime) then --dont need to do this if it was just set
			g_Legs.LegEnt.bm.t:SetModel(ply:isFemale() and defaultfemtop or defaultmaletop)
			g_Legs.LegEnt.bm.b:SetModel(ply:isFemale() and defaultfembot or defaultmalebot)
			g_Legs.LegEnt.bm.t:SetBodygroup(0, 0)
			g_Legs.LegEnt.bm.b:SetBodygroup(0, 0)
		end

		if(IsValid(g_Legs.LegEnt)&&data.top) then
			g_Legs.LegEnt.bm.t:SetBodygroup(0, data.top)
		end
		if(IsValid(g_Legs.LegEnt)&&data.bot) then
			g_Legs.LegEnt.bm.b:SetBodygroup(0, data.bot)
		end
	end
	--UNUSED CODE END
...
hook.Add("OnCharFallover", "bmfall", function(pl, rag, state)
	...
	bot = ply:getData("gbot", 0), botskin = ply:getData("gbotskin", 0),
	}
...
function nut.newchar.resetModels(ply)
	local model = ply:GetModel()
	if(!nut.newchar.isBM(model)) then return end
	...
	net.Broadcast()
		...
		clothesnonply(ply, {reset = true}, ply)
	end
end
