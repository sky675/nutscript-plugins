...
PLUGIN.spawnLevels = {
	--npcs max, this to this spawned every round, time per round
    {3, {0,1}, 300}, --used for default class
    {3, {0,1}, 1800}, --used for specific class points
}
...
function PLUGIN:SpawnZombiesAtPoint(p, amt, target, ignoremax)
    ...
    for i=1, amt do
        ...
        local ent = ents.Create(point.class or "nz_sky_synth") --unsure about what should be spawned yet so just do fast
		if(IsValid(ent) and (point.class or "nz_sky_synth") != "nz_sky_synth") then
			ent:SetPos(spawn)
