ITEM.name = "Sunrise Gas Mask"
ITEM.desc = "test appearance based on underlying suit, no suit will keep default appearance"
...
ITEM.getBodyGroups = function(item, ply)
	...
	if(!ply.bm.t:GetNoDraw()) then
		...
		for k,v in pairs(old) do
			if(ply.bm.t:FindBodygroupByName(v.name) != -1) then
