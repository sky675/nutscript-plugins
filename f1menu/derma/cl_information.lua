		...
		if (self.model) then
			...
			if (ent and IsValid(ent)) then
				...
				if(nut.newchar and nut.newchar.isBM(model)) then
					...
					if(LocalPlayer():getChar():getData("gbot", 0) == -1) then
						bot.nodraw = true
					end
					...
					timer.Create("infomodelup", 2, 0, function()
						...
						print("just wanted to check how often this runs actually")
						...
						if(panel.model.modelcache[2].nodraw and LocalPlayer():getChar():getData("gbot", 0) != -1) then
							panel.model.modelcache[2].nodraw = nil
						elseif(!panel.model.modelcache[2].nodraw and LocalPlayer():getChar():getData("gbot", 0) == -1) then
