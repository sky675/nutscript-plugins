...
PLUGIN.objs = {
	...
	[1] = {
		...
		onSpawn = function(obj, area, used) 
			...
			SpawnEnemies(mutants.list, area.mutantSpawns, used, 
				math.random(5,6), math.random(3,4), --todo scale based on player count?
				function(npc)
			...
			netstream.Start(player.GetAll(), "fakepda", "WARNING: "..mutants.name.." APPROACHING "..area.name)
	...
	[2] = {
		...
		onSpawn = function(obj, area, used) 
			...
			SpawnEnemies(enem, area.enemySpawns, used,
				...
				function(npc)
					if(!spawnedBounty && math.random(1,6) == 2) then
						...
						local tbl = {
							name = Format(job.name, job.format(npc, area, reward)),
							unique = true,
							type = "banditbounty",
							itemid = id,
							["reward"] = reward
			...
			SpawnLoot(itemSpawns, used, math.random(4,6), math.random(2,3))
