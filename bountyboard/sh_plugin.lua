...
nut.config.add("bbTime", 7200, "The length of the timer which causes the bounty board jobs to reset", nil, {
	form = "Int",
	data = {min = 600, max = 14400},
	category = "Bounty Board",
	callback = function(old, new)
		timer.Adjust("bbrefresh", new) --should work
	end
})
nut.config.add("bbMax", 4, "Max amount of jobs generated per refresh.", nil, {
	form = "Int",
	data = {min = 1, max = 12},
	category = "Bounty Board"
})
...
if(SERVER) then
	...
	function CreateJobUI(jobs, timeTil)
		...
		timeCount:SetSize(base:GetWide()/2, 64)
		timeCount:Dock(TOP)
	...
	local function LoadJobs(jobs)
		...
		for k,v in pairs(jobs) do
			...
			butt:SetDesc(PLUGIN.jobTemps[v.type].desc) --?
			butt:SetSize(jobUI.holder:GetWide(), 128)
			butt:Dock(TOP)
