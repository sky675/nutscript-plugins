...
--this was never implemented lol - table of things so theyll stay the same between restarts (button disabled, etc)
PLUGIN.mapdata = PLUGIN.mapdata or {}
--the list of available plugins, unless you intend to add one manually to an item, there needs to be an item equivalent
PLUGIN.programlist = {
	["suspend"] = {
		name = "Suspend V1.0",
		...
		--ran when its selected via dropdown in a hack
		...
		--ran after the hack is finished, win or lose
		onfinish = function()
			suspendused = nil
		end
	},
	["rangeext"] = {
		...
		--ran when installed on a hacktool
		oninstall = function(tool)
			tool:setData("hackrange", tool.hackrange * 1.5)
		end,
		--ran when uninstalled from a hacktool
...
--these are MapCreationIDs of entity classes that are hackable but these specific entities shouldnt be
local disabledents = {
...
--these are the list of default entity classes, their hack types 
--(see the actual func used for applying hacks), and the skill level required for it
local hacktypes = {
...
--these are all the default/non-customhackbase-based entities that can be hacked
...
--these func_doors can be toggled via hacking
...
--these are extra requirements needed for being hackable
local hackreq = {
	["playsound"] = { --models that can be hacked with this type
...
--this is for the playsound hack type, ["model"] = {sounds}
local playsounds = {
...
--translation for map entities so they have special names in the hack menu
local transl = {
	--can be targetname or map creation id
...
--the function used for determining the hack ent name to display, add custom ones here
...
--these class names (or map creation ids) will have that specified string as their hack entity type in the list, otherwise it will show their class
local enttrans = {
...
if(SERVER) then
	...
	--this is the func used for applying a successful hacks, 
	--if you do custom non-customhackbase ones do them among the type == checks
